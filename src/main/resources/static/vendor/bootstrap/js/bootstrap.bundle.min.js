...
!function (t, e) {
    ...
    const G = "next", J = "prev", Z = "left", tt = "right", et = "slid.bs.carousel", it = "carousel", nt = "active",
        ...
        ot = {interval: 10000e3, keyboard: !0, pause: "hover", ride: !1, touch: !0, wrap: !0}, rt = {
    ...
    class at extends z {
        ...
        _getItemIndex(t) {
            ...
            // 2023.05.05 슬라이드 input 포커스 (Changmin)
            // getIndex 함수가 슬라이드 시작 전 2번, 슬라이드 이동 후 1번 사용 되기에 아래처럼 사용
            
            if(document.getElementsByClassName("cardQuiz").item(0) != null){
            if(ddd == 0){
                ddd++;
                // setTimeout 은 500(0.5)초 이후 실행 되기 때문에 사용 (interval 과 다르게 한번만 사용됨)
                // 바로는 사용이 안됨 (아래 ddd =1일때 함수 사용과 겹쳐져서 짤림)
                // 다른 함수와 같이 사용 안한다면 바로 가능 (아래 train경우)
                setTimeout(function(){

                	t.getElementsByClassName("inputHw").item(0).focus();
                    let cardSize = 100/Number(document.getElementsByClassName("cardSize").item(0).getAttribute("value"));
                    let progBar = document.getElementsByClassName("progress-bar").item(0);
                    let progVal = Number(progBar.getAttribute("aria-valuenow"));
                    progBar.setAttribute("aria-valuenow", progVal+cardSize);
                    progBar.style.width = (progVal+cardSize)+ "%";
                ...
                t.getElementsByClassName("inputCheck").item(0).click();
               
            }
            }
            
            if(document.getElementsByClassName("train").item(0) != null){
            
            if(t.getElementsByClassName("train").item(0).textContent < 3){
            
            
            if(ddd == 0){
                ddd++;
             		
                    let oneSize = 100/Number(document.getElementsByClassName("cardSize").item(0).getAttribute("value")-1);
                    let progBar = document.getElementsByClassName("progress-bar").item(0);
                    let cardNum = t.getElementsByClassName("cardNum").item(0).textContent;
                    progBar.setAttribute("aria-valuenow", oneSize*cardNum);
                    progBar.style.width = (oneSize*cardNum)+ "%";
            ...
            if(document.getElementsByClassName("stcTrain").item(0) != null){
            
            if(t.getElementsByClassName("stcTrain").item(0).textContent <= 0){
            ...
            if(ddd == 0){
                ...
                let curBtn = setTimeout(function(){
					
					
					t.getElementsByClassName("curBtn").item(0).click();
            ...
            if(ddd == 0){
                ddd++;
                setTimeout(function(){
             		t.getElementsByClassName("inp").item(0).focus();
